...
export function formatDate(
  ...
  // parseApiDate, not new Date: the API may send a UTC timestamp with no zone
  // suffix, which a browser would read as local time.
...
export function renderIssueImage(issue: Issue): string {
  ...
  return `
      <span class="issue-card-media issue-card-media--attachment issue-card-media--${image.style}" data-preview-label="${escapeHtml(image.previewLabel)}" role="img" aria-label="${escapeHtml(image.alt)}">
        <img class="issue-card-media__image" src="${escapeHtml(image.url)}" alt="" width="720" height="480" loading="lazy" decoding="async" referrerpolicy="no-referrer">
      </span>`;
}

/** Screen readers run sentences together, so each fragment gets a full stop. */
function asAnnouncement(text: string): string {
...
export function renderIssueCard(issue: Issue): string {
  ...
  const freshUpdateLabel = String(issue.ui?.freshUpdateLabel || "New update").trim() || "New update";
  ...
  return `
      <article class="ocsp-card ocsp-card--interactive issue-card issue-card--${status.key} issue-filter-item issue-filter-item--${status.key}${hasFreshUpdate ? " is-updated" : ""}" data-issue-id="${issueDomId}">
        <button
          ...
          aria-label="View details for ${escapeHtml(asAnnouncement(issueTitle))}${hasFreshUpdate ? ` ${escapeHtml(asAnnouncement(freshUpdateLabel))}` : ""}">
...
/**
 * Lets the reporter set or replace the public image URL for their issue while
 * it is still Open or In Progress - once it is Resolved the record is closed.
 *
 * The backend stores attachments as URLs and has no upload endpoint, so this
 * takes a link. POST /attachment/Create is Citizen-only, which is why the panel
 * belongs to the citizen dialog and not the staff one.
 *
...
function renderImageUpdatePanel(issue: IssueDetail): {
  ...
  const canUpdateImage =
    ["Open", "InProgress"].includes(issue.currentStatus) && ui.imageUpdateAvailable !== false;

  if (!canUpdateImage) {
    return { action: "", panel: "" };
  }

  const panelId = `citizenIssueUpdatePanel-${issueDomId}`;
  const inputId = `citizenIssueImageUrl-${issueDomId}`;
  const helpId = `citizenIssueImageHelp-${issueDomId}`;
  const statusId = `citizenIssueImageStatus-${issueDomId}`;

  const action = `
        <button class="ocsp-button ocsp-button--cancel" type="button" data-action="toggle-issue-image-update" aria-expanded="false" aria-controls="${panelId}">
          <i class="bi bi-pencil-square" aria-hidden="true"></i>Update issue
        </button>`;

  const panel = `
        <section class="ocsp-card p-3 mb-4" id="${panelId}" hidden>
          ...
          <form data-action="update-issue-image" data-issue-id="${issueDomId}">
            <label class="form-label" for="${inputId}">Image URL</label>
            <div class="input-group">
              <span class="input-group-text"><i class="bi bi-image" aria-hidden="true"></i></span>
              <input class="form-control" id="${inputId}" name="imageUrl" type="url" inputmode="url" maxlength="300" autocomplete="url" spellcheck="false" required value="${escapeHtml(ui.editableImageUrl || "")}" placeholder="https://example.com/issue-photo.jpg" aria-describedby="${helpId} ${statusId}">
            </div>
            <p class="small text-muted mt-2 mb-3" id="${helpId}">Use a public address beginning with http:// or https://.</p>
            <p class="small mb-3" id="${statusId}" data-image-update-status role="status" aria-live="polite"></p>
...
export function renderIssueDetailModal(issue: IssueDetail): string {
  ...
  const imageUpdate = renderImageUpdatePanel(issue);
