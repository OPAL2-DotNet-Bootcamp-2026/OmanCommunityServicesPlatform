(function initializeOcspDashboardRenderers(global) {
  "use strict";

  const ocsp = global.OCSP || {};
  const shared = ocsp.issueRenderers;

  function statusCardClass(statusKey) {
    return {
      open: "open",
      progress: "inprogress",
      resolved: "resolved"
  ...
  function renderStaffIssueCard(issue) {
    const status = shared.getStatusMeta(issue.currentStatus);
    const priority = shared.getPriorityMeta(issue.priority);
    const issueDomId = shared.safeDomId(issue.issueId);
          ...
          <a
            ...
            <div class="row align-items-center w-100 me-2 g-2 text-start">
              <div class="col-12 col-md-3 d-flex align-items-center gap-3">
                ${shared.renderIssueImage(issue)}
                <div class="min-w-0">
                  <h3 class="issue-title mb-0 fw-bold text-dark text-truncate">${shared.escapeHtml(issue.title)}</h3>
                  <small class="text-muted">REQ-${shared.escapeHtml(issue.issueId)}</small>
                </div>
              </div>
              <div class="col-6 col-md-2 text-dark small">
                User ID: ${shared.escapeHtml(issue.reportedById || "Unavailable")}
              </div>
              <div class="col-12 col-md-3 text-muted small">
                <span class="fw-semibold text-dark">${shared.escapeHtml(department)}</span><br>
                <span class="text-secondary">${shared.escapeHtml(category)} &bull; ${shared.escapeHtml(region)}</span>
              </div>
              <div class="col-6 col-md-2 text-md-center d-flex justify-content-md-center gap-2 flex-wrap status-priority-group">
                <span class="status-badge status-badge--${status.key}">${shared.escapeHtml(status.label)}</span>
                <span class="priority-badge priority-badge--${priority.key}">${shared.escapeHtml(priority.label)}</span>
              </div>
              <time class="col-12 col-md-2 text-md-end text-muted issue-date" datetime="${shared.escapeHtml(issue.reportedDate || "")}">
                ${shared.escapeHtml(shared.formatDate(issue.reportedDate))}
              </time>
            </div>
          </a>
        </div>
  ...
  function renderStaffRatingPanel(rating) {
    if (!rating) {
      return `
    ...
    const stars = [1, 2, 3, 4, 5]
      .map((value) => `<i class="bi ${value <= score ? "bi-star-fill" : "bi-star"}" aria-hidden="true"></i>`)
      .join("");
    const feedback = rating.feedback
      ? `<p class="mb-0 small text-muted">&ldquo;${shared.escapeHtml(rating.feedback)}&rdquo;</p>`
  ...
  function renderStaffActionPanel(issue) {
    const issueDomId = shared.safeDomId(issue.issueId);
    ...
    const options = issue.currentStatus === "Open"
  ...
  function renderCompactTimeline(statusUpdates) {
    return shared.renderTimeline(statusUpdates).replace(
      'class="activity-timeline"',
      'class="activity-timeline activity-timeline--compact"'
    );
  }

  function renderStaffIssueDetailModal(issue) {
    const issueDomId = shared.safeDomId(issue.issueId);
    const mapName = issue.regionName || "Issue location";
    const hasLatitude = issue.latitude !== null && issue.latitude !== undefined && issue.latitude !== "";
    const hasLongitude = issue.longitude !== null && issue.longitude !== undefined && issue.longitude !== "";
    const latitude = hasLatitude ? Number(issue.latitude) : NaN;
    const longitude = hasLongitude ? Number(issue.longitude) : NaN;
    const coordinateCopy = Number.isFinite(latitude) && Number.isFinite(longitude)
    ...
    const warningAlert = warnings.length
      ? `
        <div class="alert alert-warning small mb-4" role="alert">
          ...
          ${warnings.map(shared.escapeHtml).join(", ")}.
        </div>`
      : "";
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable modal-fullscreen-sm-down issue-detail-dialog">
          <div class="modal-content issue-detail-modal__content">
            <div class="modal-header issue-dialog__header issue-detail-modal__header">
              <div>
                <span class="content-label">STAFF ISSUE &middot; REQ-${shared.escapeHtml(issue.issueId)}</span>
                <h2 class="modal-title" id="issueModalLabel-${issueDomId}">${shared.escapeHtml(issue.title)}</h2>
              </div>
            ...
            <div class="modal-body issue-detail-modal__body bg-white p-4">
              ...
              <div class="row">
                <div class="col-md-7 pe-md-4 border-end">
                  <p class="mb-2"><strong>Description:</strong> ${shared.escapeHtml(issue.description)}</p>
                  <p class="mb-3 text-muted small">
                    <i class="bi bi-geo-alt-fill me-1" aria-hidden="true"></i>
                    ${shared.escapeHtml(issue.location)}
                  </p>
                  <div class="mb-4 rounded overflow-hidden border bg-light d-flex align-items-center justify-content-center issue-map-preview" role="img" aria-label="Map placeholder showing ${shared.escapeHtml(mapName)}">
                    <div class="text-muted text-center">
                      <i class="bi bi-map text-secondary mb-2 issue-map-preview__icon" aria-hidden="true"></i>
                      <p class="mb-0">${shared.escapeHtml(mapName)}<br><small>${shared.escapeHtml(coordinateCopy)}</small></p>
                    </div>
                  </div>
                  <div class="attachments-section">
                    <span class="content-label"><i class="bi bi-paperclip me-1" aria-hidden="true"></i>Citizen Attachments</span>
                    ${shared.renderAttachments(issue.attachments)}
                  </div>
                  <div class="mt-4">
                ...
                <div class="col-md-5 ps-md-4 comments-column">
                  ...
                  <div class="comment-thread mb-3" data-comment-thread aria-label="Issue updates and comments" role="list">
                    ${shared.renderComments(issue.comments)}
                  </div>
  ...
  ocsp.dashboardRenderers = Object.freeze({
